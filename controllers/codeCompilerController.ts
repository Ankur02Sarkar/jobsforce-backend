...
export const submitCode = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!languageIds[language as keyof typeof languageIds]) {
    return res.status(400).json({
      success: false,
      message: "Invalid language. Supported languages: javascript, python, c, cpp, java",
    });
  }
  ...
  formData.append("widget_form[compiler]", languageIds[language as keyof typeof languageIds].toString());
  ...
  try {
    ...
    const submitData = await submitResponse.json() as SphereEngineSubmitResponse;
 ...
 * @access  Public
 */
export const getCodeResult = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const resultData = await resultResponse.json() as SphereEngineStatusResponse;
