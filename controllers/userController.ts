    ...
    try {
      ...
      return res.status(400).json({
        ...
        error: {
          ...
          details: process.env.NODE_ENV === 'development' ? { error: (error as Error).message } : undefined
        }
      });
    }
      ...
      try {
        ...
        return res.status(500).json({
          ...
          error: {
            ...
            details: process.env.NODE_ENV === 'development' ? { error: (dbError as Error).message } : undefined
          }
        });
      }
    }
  ...
  try {
    ...
    return res.status(500).json({
      ...
      error: {
        code: 'SERVER_ERROR',
        message: 'An unexpected error occurred',
        details: process.env.NODE_ENV === 'development' ? { errorMessage: (error as Error).message } : undefined
      }
    });
  }
}); 